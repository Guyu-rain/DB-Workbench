...
class StorageEngine {
 public:
  ...
  // Backup/Restore
  bool BackupDatabase(const std::string& dbName, const std::string& backupName, uint64_t checkpointLsn, std::string& err);
  bool RestoreDatabase(const std::string& dbName, const std::string& backupName, std::string& err);
