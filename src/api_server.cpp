...
void ApiServer::HandleSqlConsole(const HttpRequest& req, HttpResponse& resp) {
    (void)req;
    std::string html = ReadFileIfExists("../docs/sql_console.html");
    if (html.empty()) html = ReadFileIfExists("../../docs/sql_console.html");
    if (html.empty()) {
        resp.status = 404;
        ...
        if (cmd.type == CommandType::kDropDatabase) {
           ...
           if (!engine_.DropDatabase(cmd.dbName, err)) {
                ...
                if (currentDbName_ == cmd.dbName) {
                    currentDbName_ = "default";
                    currentDbf_ = "MyDB.dbf";
                    currentDat_ = "MyDB.dat";
...
void ApiServer::HandleUseDatabase(const HttpRequest& req, HttpResponse& resp) {
    ...
    // �޸���ȷ�����ɾ���·��
    std::string baseDir;
    size_t lastSlash = currentDbf_.find_last_of("/\\");
    if (lastSlash != std::string::npos) {
        baseDir = currentDbf_.substr(0, lastSlash + 1); // ��ȡĿ¼����
    }
    else {
        baseDir = "./"; // Ĭ��Ϊ��ǰĿ¼
    }
...
void ApiServer::HandleListDatabases(const HttpRequest& req, HttpResponse& resp) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            ...
            // Hide system database
            if (filename == "system.dbf") continue;

            if (filename.length() > 4) {
                dbNames.push_back(filename.substr(0, filename.length() - 4));
            }
...
void ApiServer::HandleLoginPage(const HttpRequest& req, HttpResponse& resp) {
    (void)req;
    std::string html = ReadFileIfExists("../docs/login.html");
    if (html.empty()) html = ReadFileIfExists("../../docs/login.html");
    if (html.empty()) html = ReadFileIfExists("docs/login.html");
    
    if (html.empty()) {
        resp.status=404; resp.body="Login page not found"; return;
    }
    resp.status = 200;
    resp.body = html;
    resp.contentType = "text/html; charset=utf-8";
...
void ApiServer::HandleIndex(const HttpRequest& req, HttpResponse& resp) {
  (void)req;
  std::string html = ReadFileIfExists("../docs/workbench.html");
  if (html.empty()) html = ReadFileIfExists("../../docs/workbench.html");
  if (html.empty()) {
    resp.status = 404;
